        ...
        if (contactResult.getSuccess()) {
            ...
            for (var i in capContacts) {
                ...
                if(inspType == "3000 Final Building Permit" && (appMatch("Building/Commercial/New Construction/NA")
                    ...
                    && vBalanceDue <= 0 && inspResult == "Pass-Revised")
                {
...
//CASANLEAN-1554
    if(inspType == "2050 Electrical Service Release" && inspResult == "Pass-Revised") {
        try
        {
            var flag = false;
            var params = aa.util.newHashtable();
    ...
    if(inspType == "2060 Gas Service Release" && inspResult == "Pass-Revised") {
        try
        {
            var flag = false;
            var params = aa.util.newHashtable();
...
//CASANLEAN-1496
    if( inspResult == "Pass-Revised" || inspResult == "Fail-Revised")
    {
        var date = new Date();
        ...
        if(inspType == "Courtesy" && !feeExists("XCTY","NEW","INVOICED"))
        {
            ...
            if (contactResult.getSuccess()) {
                ...
                for (var i in capContacts) {
                    ...
                    addParameter(params, "$$InspectionType$$", inspType);
                    addParameter(params, "$$InspectionResultComment$$", inspComment);
                    addParameter(params, "$$ACUrl$$", String(lookup("ACA_CONFIGS", "ACA_SITE")).split("/Admin")[0]);
                    if(hm[applicantEmail+""] != 1)
                    {
                        sendEmail("no-reply@sanleandro.org", applicantEmail, "", "BLD_INSPECTION_RESULT_OVERTIME_COURTESY", params, VRFiles, capId);
                        hm[applicantEmail+""] = 1;
                    }
                }
            }
