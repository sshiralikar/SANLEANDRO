/*------------------------------------------------------------------------------------------------------/
| Program : ACA_BEFORE_VALIDATE_LP_CLASS.js
...
//TESTING
// var testingCap = aa.cap.getCapID("23TMP-000320").getOutput();
    ...
    if(lpList && lpList.toArray) {
        ...
        for(var lpIndex in lpList) {
            ...
            if(licType == "Contractor") {
                var validObj = validateCSLBClassifications(licNum, appTypeString);
                //aa.print(validObj.validated);
                //aa.print(validObj.message);
                if(validObj.validated) {
                    errors = [];
                    break;
                }
                if(!validObj.validated) {
                    errors.push(validObj.message);
                }
            }
        }        
        if(errors.length > 0)  {
