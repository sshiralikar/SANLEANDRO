    ...
    (function () {
        ...
        */
        for(var resultsIndex in cslbResults) {
            ...
            if(cslbResultObj.createdReference) {
                formMessage.push("<p>Reference license professional has created</p>");
            }
            if(cslbResultObj.syncedReference) {
                formMessage.push("<p>Reference license professional has been sycned with CSLB</p>");
            }
...
function insertColumn(tableObj, columnName) {
    tableObj.columns.push("<th>" + columnName + "</th>");
}

function insertRow(tableObj, values) {
    tableObj.rows.push(values.map(function(data) {
        return "<td>" + (data ? data : "N/A") + "</td>";
    }));
}

function renderTable(tableObject) {
    var tableHTML = "<table cellpadding='100' cellspacing='100' border='1'><tr>"
    tableHTML += tableObject.columns.join("");
    tableHTML += "</tr>";
    tableHTML += tableObject.rows.map(function (data) {
        return "<tr>" + data + "</tr>";
    })
    tableHTML += "</table>";
    return tableHTML;
